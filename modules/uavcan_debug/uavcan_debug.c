...
void uavcan_send_debug_msg(uint8_t debug_level, char* source, const char *fmt, ...)
{
    ...
    uavcan_broadcast_all(&uavcan_protocol_debug_LogMessage_descriptor, CANARD_TRANSFER_PRIORITY_LOWEST, &log_msg);
}
...
void uavcan_send_debug_keyvalue(char* key, float value)
{
    ...
    uavcan_broadcast_all(&uavcan_protocol_debug_KeyValue_descriptor, CANARD_TRANSFER_PRIORITY_LOWEST, &log_kv);
}
